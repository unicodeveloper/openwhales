...
import { useFundChat } from "@/hooks/use-fund-chat";
...
const SUGGESTED_PROMPTS = [
  {
    label: "Top holdings",
    prompt:
      "What are this fund's highest-conviction positions? Analyze position sizing relative to total portfolio value.",
    icon: Briefcase,
  },
  {
    label: "Recent moves",
  ...
  },
  {
    ...
    prompt:
      "What sectors and themes is this fund concentrated in? Is there a visible strategy shift?",
    icon: Building2,
  },
];

export function FundChat({ fund, fundData, isOpen, onClose }: FundChatProps) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLTextAreaElement>(null);

  const { messages, input, setInput, sendMessage, isStreaming, error } =
    useFundChat({ fund, fundData });
  ...
  // Short display name: take last word or first two words
  const displayName = fund.split(" ").length > 2
    ? fund.split(" ").slice(0, 2).join(" ")
    : fund;
        ...
        >
          {!hasMessages && (
            <div className="h-full flex flex-col items-center justify-center">
              <div className="w-10 h-10 rounded-full bg-blue-500/5 border border-blue-500/10 flex items-center justify-center mb-3">
              ...
              <div className="grid grid-cols-2 gap-1.5 mt-4 w-full max-w-sm">
                {SUGGESTED_PROMPTS.map((sp) => (
                  <button
                    ...
                    className="group/card flex items-start gap-2 p-2.5 rounded-lg border border-border/40 hover:border-blue-500/30 hover:bg-blue-500/5 transition-all text-left cursor-pointer"
                  >
                    <sp.icon className="h-3 w-3 text-muted-foreground group-hover/card:text-blue-400 transition-colors mt-0.5 shrink-0" />
            ...
            <div key={message.id}>
              {message.role === "user" ? (
                <div className="flex justify-end">
                  <div className="max-w-[85%] px-3 py-2 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs leading-relaxed">
                    {message.content}
                  </div>
                </div>
              ) : (
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1 relative">
                <textarea
                  ...
                  className="w-full resize-none rounded-lg border border-border/60 bg-muted/20 px-3 py-2.5 pr-8 text-xs placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/40 focus:ring-1 focus:ring-blue-500/20 disabled:opacity-50 transition-colors"
                />
                {input && !isStreaming && (
                  <button
                    type="button"
              ...
              <button
                ...
                className="shrink-0 h-[38px] w-[38px] rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 hover:bg-blue-500/20 hover:border-blue-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center justify-center cursor-pointer"
              >
                {isStreaming ? (
...
/** CTA button to open the fund chat */
export function FundChatCTA({
  onClick,
  variant = "default",
}: {
  onClick: () => void;
  ...
  if (variant === "header") {
    return (
      <button
        onClick={onClick}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-blue-500/30 bg-blue-500/5 text-xs text-blue-400 font-medium hover:bg-blue-500/10 hover:border-blue-500/40 transition-all cursor-pointer"
      >
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-blue-500/30 bg-blue-500/5 text-[11px] text-blue-400 font-medium hover:bg-blue-500/10 hover:border-blue-500/40 transition-all cursor-pointer"
    >
      <MessageSquare className="h-3 w-3" />
      Deep dive chat
    </button>
