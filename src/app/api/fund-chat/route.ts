...
const SYSTEM_PROMPT = `You are OpenWhales Alpha, an elite financial research assistant. The user is researching a specific fund or investment firm and wants deep insight into its strategy, holdings, and moves.

Your capabilities:
- Search SEC filings (13F, 13D/G, Form 4, 10-K, 10-Q, 8-K, proxy statements) for this fund's holdings, position changes, activist stakes, and insider trades
- Search financial data for market context on stocks this fund holds
- Research companies in this fund's portfolio for competitive landscape and fundamentals
...
export async function POST(request: Request) {
  try {
    ...
    const rl = checkRateLimit(`fund-chat:${ip}`, { maxRequests: 20, windowSeconds: 60 });
    ...
    const { messages, fund, context } = parsed.data;

    const systemWithContext = context
      ? `${SYSTEM_PROMPT}\n\nThe user is researching the fund "${fund}". Here is the current data already displayed on their screen:\n\n${context}\n\nUse this as background context, but always fetch fresh data with tools when the user asks specific questions.`
      : `${SYSTEM_PROMPT}\n\nThe user is researching the fund "${fund}".`;
    ...
    console.error("Fund chat API error:", error);
    return new Response(
